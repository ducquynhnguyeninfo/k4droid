...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public interface OnPageChangedListener extends
	...
	private int dividerPadding = 12; // from the top of the view with the
										// divider
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		// Get size of system displaying windows
	...
	private void addTextTab(final int position, String title) {
		...
		// set the width of the tabpadding
		float densityMuntiplier = getContext().getResources()
				.getDisplayMetrics().density;
		float scaled = ((TextView) tab).getTextSize() * densityMuntiplier;
		Paint p = tab.getPaint();
		p.setTextSize(scaled);
		float lengthTextInPixel = (float) p.measureText(((TextView) tab)
				.getText().toString());
		tabPadding = (int) ((float) ((measuredwidth / tabCount - lengthTextInPixel) + scrollOffset
				/ (tabCount - 1)) / (2) + (dividerWidth * (tabCount - 1)));
		_Log.v(tab.getText().toString(), measuredwidth + "   " + tabCount);
		_Log.v(tab.getText().toString(), measuredwidth / tabCount + " "
				+ lengthTextInPixel + " " + tabPadding);
		addTab(position, tab);
	}
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset,
				int positionOffsetPixels) {
			if (context instanceof PagerSlidingTabStrip.OnPageChangedListener) {
				...
				if (context instanceof PagerSlidingTabStrip.OnPageChangedListener) {
					((PagerSlidingTabStrip.OnPageChangedListener) context).onPageScrolled(position, positionOffset, positionOffsetPixels);
				}
			}
		}
		...
		public void onPageScrollStateChanged(int state) {
			...
			if (context instanceof PagerSlidingTabStrip.OnPageChangedListener) {
				((PagerSlidingTabStrip.OnPageChangedListener) context)
